...
func (m *Manager) InitialContainer() error {
	...
	for _, containerItem := range containerList {
		...
		_, ok := ContainerCurrnetDetail[ID]

		if ok == false {
			tmpDetail := &ContainerResource{CtnLabels: CtnLabels{OriginalLabels: labels}}
			ContainerCurrnetDetail[ID] = tmpDetail
		}
	}
	return nil
...
func (m *Manager) AdjustContainer(containerID string, action string) error {
	...
	if action == "add" {
		...
		if ok == false {
			labels, err := m.GetLabels(containerID)
			if err != nil {
				return err
			}
